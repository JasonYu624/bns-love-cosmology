...
def get_args():
    ...
    parser.add_argument("--pop-outdir", default="outputs/outdir_population_SEOBNR")
    parser.add_argument("--event-index", type=int, default=1)
    parser.add_argument("--outdir", default="outputs/outdir_population_run_SEOBNR")
    ...
    parser.add_argument("--sigma-q", type=float, default=0.0957,
                        help="Scale of the reflected half-Gaussian mass-ratio prior (= sqrt(2)*sigma_m/mu).")
    parser.add_argument("--q-min", type=float, default=0.5,
                        help="Lower truncation of the mass-ratio prior.")
    parser.add_argument("--spin-mode", choices=["sample", "fix", "flat"], default="sample",
                        help="sample=AlignedSpin baseline (spin cancels in HyperPE); "
                             "fix=DeltaFunction at injected chi (diagnostic); "
                             "flat=Uniform chi_z with prior-column spin reweight (option b).")
    parser.add_argument(
        "--sky-frame",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Backward-compatibility flag only. This script always uses posterior samples for reweighting.",
    )
    parser.add_argument(
        "--rw-clip-weights",
        action="store_true",
        default=False,
        help="Clip log-weights before exponentiating. Default follows bilby and does not clip.",
    )
    parser.add_argument(
        "--rw-clip-min-log",
        type=float,
        default=-745.0,
        help="Lower clip bound applied only when --rw-clip-weights is set.",
    )
    parser.add_argument(
        "--rw-use-cache",
        action="store_true",
        default=False,
        help="Reuse cached reweight arrays if available and compatible.",
    )
    parser.add_argument(
        "--rw-cache-file",
        default=None,
        help="Optional path for cached reweight arrays (.npz).",
    )
...
EXPECTED_WAVEFORM_APPROXIMANT = "SEOBNRv5_ROM_NRTidalv3"
actual_waveform_approximant = str(waveform_arguments_full.get("waveform_approximant", ""))
if actual_waveform_approximant != EXPECTED_WAVEFORM_APPROXIMANT:
    raise ValueError(
        f"Expected waveform_approximant={EXPECTED_WAVEFORM_APPROXIMANT!r} from catalog_config.json, "
        f"but got {actual_waveform_approximant!r}. Check --pop-outdir."
    )
...
RW_USE_NESTED = False
RW_CLIP_WEIGHTS = bool(args.rw_clip_weights)
RW_CLIP_MIN_LOG = float(args.rw_clip_min_log)
RW_USE_CACHE = bool(args.rw_use_cache)
...
CACHE_FILE = args.rw_cache_file or os.path.join(outdir, f"{RW_LABEL}_reweight_arrays.npz")

print(f"LAL_DATA_PATH={os.environ.get('LAL_DATA_PATH', 'not_set')}", flush=True)
print(f"Waveform approximant from catalog: {waveform_arguments_full.get('waveform_approximant', 'unknown')}", flush=True)

if args.rw_use_nested_samples:
    print(
        "Warning: --rw-use-nested-samples was requested, but this script is configured to use posterior samples only.",
        flush=True,
    )
...
# 4. Priors
# =========================================================
class UniformInComponentsMassRatioReflected(bilby.gw.prior.UniformInComponentsMassRatio):
    """UniformInComponentsMassRatio with a boundary-reflected prior transform.

    The MARGINAL prior is identical to UniformInComponentsMassRatio (verified
    bin-for-bin); only the unit-cube -> q mapping used by the nested sampler
    changes.  The fold u -> 2*min(u, 1-u) sends q = q_max (=1) to the interior
    (u=0.5) of the dynesty sampling cube instead of its hard edge.  For the
    near-symmetric (q~1) systems whose posteriors pile up against q=1, this
    avoids the bounding-volume mis-estimation at the cube boundary that
    otherwise biases q low.  Because the marginal is unchanged, this does not
    make the prior informative; it only addresses a sampler geometry artifact.
    """

    def rescale(self, val):
        u = 2.0 * np.minimum(val, 1.0 - val)
        return super().rescale(u)


class TruncatedGaussianReflected(bilby.core.prior.TruncatedGaussian):
    """TruncatedGaussian with the same boundary-reflected unit-cube transform as
    UniformInComponentsMassRatioReflected.  Used as an informative, population-matched
    mass-ratio prior peaked at q=1: a Gaussian centred at mu=1 truncated at maximum=1 is
    exactly a half-Gaussian.  The fold u -> 2*min(u, 1-u) maps the q=1 peak (the cube edge
    where near-equal-mass posteriors pile up) into the cube interior, fixing the dynesty
    bounding-volume artifact.  The fold sends Uniform(val) -> Uniform(u), so the MARGINAL
    prior is unchanged (still the truncated half-Gaussian); only the sampler geometry differs.
    """
...
def make_priors():
    ...
    # --- Spin prior (variant-dependent) ---
    if SPIN_MODE == "fix":
        # Option a (diagnostic): fix aligned spins at the injected truth (no spin sampling).
        p["chi_1"] = bilby.core.prior.DeltaFunction(peak=float(inj.get("chi_1", 0.0)), name="chi_1")
        p["chi_2"] = bilby.core.prior.DeltaFunction(peak=float(inj.get("chi_2", 0.0)), name="chi_2")
    elif SPIN_MODE == "flat":
        # Option b: flat aligned-spin (chi_z) prior over [-0.05, 0.05]; the mismatch vs the
        # AlignedSpin injection distribution is corrected in the recycling-prior column.
        ...
        # Baseline: AlignedSpin == the injection spin distribution (spin cancels in HyperPE).
    ...
    # --- Chirp mass: plain Uniform over the narrow window (not UniformInComponents) ---
    ...
    # --- Mass ratio: informative reflected half-Gaussian peaked at q=1 (population-matched) ---
    p["mass_ratio"] = TruncatedGaussianReflected(
        mu=1.0,
        sigma=SIGMA_Q,
        minimum=Q_MIN,
        maximum=1.0,
...
def add_hyper_input_columns(df):
    ...
    # --- Mass-sector PE prior in detector-frame component masses ---
    # chirp_mass ~ Uniform and mass_ratio ~ half-Gaussian are no longer uniform-in-components,
    # so pi_PE,mass(m1,m2) = pi(Mc,q) / |d(m1,m2)/d(Mc,q)| is NOT constant.
    # Verified: |d(m1,m2)/d(Mc,q)| = Mc * (1+q)^(2/5) * q^(-6/5).
    q_det = df["mass_2"] / df["mass_1"]
    Mc_det = (df["mass_1"] * df["mass_2"]) ** 0.6 / (df["mass_1"] + df["mass_2"]) ** 0.2
    pi_mass = (
        np.exp(-0.5 * ((1.0 - q_det) / SIGMA_Q) ** 2)   # half-Gaussian q prior (unnormalised)
        * q_det ** 1.2 * (1.0 + q_det) ** (-0.4)        # / |J|_q  ->  q^(6/5) (1+q)^(-2/5)
        / Mc_det                                         # plain-Uniform Mc  ->  1/Mc factor
    )

    # --- Spin-sector correction (flat-chi variant only) ---
    # PE prior is flat in chi_z; population/injection spin is AlignedSpin(U(0,0.05)).
    # Fold pi_PE,spin / p_inj,spin = const / [p_AS(chi_1) p_AS(chi_2)] into the prior column.
    if SPIN_MODE == "flat":
        for c in ("chi_1", "chi_2"):
            if c not in df.columns:
                raise KeyError(f"spin-mode=flat needs '{c}' in the posterior columns.")
        _as = bilby.gw.prior.AlignedSpin(name="_as", a_prior=bilby.core.prior.Uniform(0.0, 0.05))
        # AlignedSpin pdf diverges (log) at chi_z=0 and ->0 at |chi_z|=a_max; clip to keep the
        # recycling prior finite & positive. The bounds are essentially never active for real
        # (chi_z within ~1e-5 of 0 or of +/-0.05); they only guard against export crashes.
        p1 = np.clip(_as.prob(df["chi_1"].to_numpy(dtype=float)), 1e-3, 1e3)
        p2 = np.clip(_as.prob(df["chi_2"].to_numpy(dtype=float)), 1e-3, 1e3)
        spin_factor = 1.0 / (p1 * p2)
    else:
        spin_factor = 1.0
...
def save_corner_and_csv(res, out_label, include_redshift=True):
    ...
    # Drop degenerate (constant) columns: corner cannot set axis limits on a
    # zero-width parameter, e.g. spins fixed at truth under --spin-mode fix.
    plot_params = [p for p in plot_params if post_df[p].nunique() > 1]
...
def summarize_log_weights(lnw, clip_weights=False, clip_min_log=-745.0):
    ...
    lnw_f = np.asarray(lnw[finite], dtype=float)
    lnw_shift = lnw_f - np.max(lnw_f)
    if clip_weights:
        lnw_shift = np.clip(lnw_shift, clip_min_log, 0.0)
    ...
    return weights, {
        ...
        "clip_weights": bool(clip_weights),
        "clip_min_log": float(clip_min_log) if clip_weights else None,
    }
...
def save_reweight_cache(cache_path, posterior, lnw, new_ll, new_lp, old_ll, old_lp, metadata):
    posterior_dict = {c: np.asarray(posterior[c]) for c in posterior.columns}
    np.savez_compressed(
        cache_path,
        posterior_dict=np.array([posterior_dict], dtype=object),
        ln_weights=np.asarray(lnw, dtype=float),
        new_log_likelihood=np.asarray(new_ll, dtype=float),
        new_log_prior=np.asarray(new_lp, dtype=float),
        old_log_likelihood=np.asarray(old_ll, dtype=float),
        old_log_prior=np.asarray(old_lp, dtype=float),
...
def expected_reweight_metadata():
    return dict(
        ...
        rw_label=RW_LABEL,
        sky_frame=SKY_FRAME,
        reference_frame=("detector" if SKY_FRAME == "detector" else "sky"),
        distance_marginalization=bool(LIKELIHOOD_SETTINGS["distance_marginalization"]),
        phase_marginalization=bool(LIKELIHOOD_SETTINGS["phase_marginalization"]),
        time_marginalization=bool(LIKELIHOOD_SETTINGS["time_marginalization"]),
        use_nested_samples=False,
        old_waveform_source_model="lal_binary_neutron_star_relative_binning",
        new_waveform_source_model="lal_binary_neutron_star",
        waveform_approximant=str(waveform_arguments_full.get("waveform_approximant", "unknown")),
        lal_data_path=os.environ.get("LAL_DATA_PATH", ""),
        pop_outdir=POP_OUTDIR,
        signal_path=SIGNAL_PATH,
    )


def cache_matches_expected(metadata, expected):
    for key, value in expected.items():
        if metadata.get(key) != value:
            return False, f"{key}: cached={metadata.get(key)!r}, expected={value!r}"
    return True, "ok"


def compute_reweight_arrays(result_obj, old_likelihood, new_likelihood, cache_path=None, use_cache=False):
    expected_meta = expected_reweight_metadata()

    if use_cache and cache_path and os.path.exists(cache_path):
        posterior, lnw, new_ll, new_lp, old_ll, old_lp, metadata = load_reweight_cache(cache_path)
        ok, reason = cache_matches_expected(metadata, expected_meta)
        if ok:
            print(f"Using cached reweight arrays from {cache_path}", flush=True)
            res = copy.copy(result_obj)
            res.posterior = posterior.copy()
            return res, posterior, lnw, new_ll, new_lp, old_ll, old_lp
        else:
            print(f"Ignoring cache due to metadata mismatch: {reason}", flush=True)
    ...
    if cache_path:
        ...
        print(f"Saved reweight cache to: {cache_path}", flush=True)

    return res, posterior, np.asarray(lnw, dtype=float), new_ll, new_lp, old_ll, old_lp


def reweight_posterior_weighted(result_obj, old_likelihood, new_likelihood):
    res, posterior, lnw, new_ll, new_lp, old_ll, old_lp = compute_reweight_arrays(
        result_obj,
        old_likelihood,
        new_likelihood,
        cache_path=CACHE_FILE,
        use_cache=RW_USE_CACHE,
    )

    weights, diag = summarize_log_weights(
        lnw,
        clip_weights=RW_CLIP_WEIGHTS,
        clip_min_log=RW_CLIP_MIN_LOG,
    )
    diag["resume_file"] = RESUME_FILE
    diag["cache_file"] = CACHE_FILE
    diag["used_cache"] = bool(RW_USE_CACHE and os.path.exists(CACHE_FILE))
    ...
    res.meta_data["reweight_cache_file"] = CACHE_FILE
    res.meta_data["reweight_clip_weights"] = RW_CLIP_WEIGHTS
    res.meta_data["reweight_clip_min_log"] = RW_CLIP_MIN_LOG if RW_CLIP_WEIGHTS else None
...
print(
    ...
    f"SKY_FRAME={SKY_FRAME}, RW_METHOD={RW_METHOD}, RW_USE_NESTED=False, "
    f"RW_CLIP_WEIGHTS={RW_CLIP_WEIGHTS}, RW_USE_CACHE={RW_USE_CACHE}",
    flush=True,
)
print("Using posterior samples only.", flush=True)
...
summary = {
    ...
    "use_nested_samples": False,
    "clip_weights": RW_CLIP_WEIGHTS,
    "clip_min_log": RW_CLIP_MIN_LOG if RW_CLIP_WEIGHTS else None,
    "use_cache": RW_USE_CACHE,
