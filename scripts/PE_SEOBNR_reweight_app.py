...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Seed for dynesty/bilby sampling only. This does not change the noise seed from meta.json.",
    )
    parser.add_argument(
        "--reweight-resample-seed",
        type=int,
        default=None,
        help="Seed for the final weighted posterior resampling. Default is sampling_seed + 100000.",
    )
    parser.add_argument(
        "--resume",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Resume dynesty from checkpoint if available. Use --no-resume for a fresh run.",
    )
    parser.add_argument(
        "--sky-frame",
...
SAMPLING_SEED = (
    ...
    else 1000000 + int(EVENT_INDEX)
)
RW_RESAMPLE_SEED = (
    int(args.reweight_resample_seed)
    if args.reweight_resample_seed is not None
    else SAMPLING_SEED + 100000
)
...
print(f"Noise seed from meta.json: {noise_seed}", flush=True)
print(f"Sampling seed: {SAMPLING_SEED}", flush=True)
print(f"Reweight resampling seed: {RW_RESAMPLE_SEED}", flush=True)
...
# Keep the data/noise fixed by using noise_seed above, then reset RNGs for sampler-only randomness.
bilby_seed(SAMPLING_SEED)
...
summary = {
    ...
    "reweight_resample_seed": int(RW_RESAMPLE_SEED),
    "rw_label": RW_LABEL,
    "rw_method": RW_METHOD,
    "use_nested_samples": False,
    "clip_weights": RW_CLIP_WEIGHTS,
