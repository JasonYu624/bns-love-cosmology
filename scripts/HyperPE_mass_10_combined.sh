#!/bin/bash -l
#SBATCH --job-name=HyperPE_Mass_SEOBNR_10
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
SCRIPT="scripts/HyperPE_mass.py"

PE_OUTDIR="${RUNDIR}/outputs/outdir_population_run_SEOBNR_combined_10"
...
OUTDIR="${RUNDIR}/outputs/outdir_mass_only_seobnr_10_combined"
LABEL="mass_only_seobnr_10_combined"
...
echo "REDSHIFT_Z_MAX=${REDSHIFT_Z_MAX}"
echo "RESUME_FLAG=${RESUME_FLAG}"
echo "Fixed cosmology is set inside ${SCRIPT}: Planck18.H0.value, Planck18.Om0, w0=-1, lamb=0."
echo "================================================="
...
ARGS=(
  ...
  --nlive "${NLIVE}"
  --seed "${SEED}"
  --redshift-z-max "${REDSHIFT_Z_MAX}"
  --postprocess-max-samples "${POSTPROCESS_MAX_SAMPLES}"
  "${RESUME_FLAG}"
)

echo "================================================="
echo "Starting mass-only SEOBNR run with first ${EVENT_LIMIT} events"
echo "Only sampled hyperparameters: mu_m, sigma_m"
echo "================================================="
...
echo "Advisor scatter matrix: ${OUTDIR}/scatter_matrix_${LABEL}_sampled_params_loglike_variance.png"
echo "================================================="
