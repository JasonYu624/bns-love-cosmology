...
OUTDIR = os.path.join(RUNDIR, "outputs/outdir_mass_only_seobnr")
LABEL = "mass_only_seobnr"
NLIVE = 500
SEED = 12345
PE_DELTA_SIGMA = 1.0 
POSTPROCESS_MAX_SAMPLES = 5000
REDSHIFT_Z_MAX = 0.3
FIXED_H0 = float(Planck18.H0.value)
FIXED_OM0 = float(Planck18.Om0)
FIXED_W0 = -1.0
FIXED_LAMB = 0.0


def get_args():
    p = argparse.ArgumentParser(description="Mass-distribution-only HyperPE pipeline with fixed Planck18 cosmology")
...
def write_postprocessing_checks(
    ...
    scatter_basic_path = os.path.join(outdir, f"scatter_matrix_{label}_mass_variance.png")
    scatter_components_path = os.path.join(outdir, f"scatter_matrix_{label}_variance_components.png")
    scatter_all_path = os.path.join(outdir, f"scatter_matrix_{label}_massparams_variance_components.png")
    ...
    _plot_scatter_matrix(
        full_posterior,
        ["mu_m", "sigma_m", "log_likelihood", "variance"],
        scatter_basic_path,
    ...
    metadata = {
        ...
        "fixed_cosmology": {
            ...
            "w0": FIXED_W0,
            "lamb": FIXED_LAMB,
            "source": "wcosmo.astropy.Planck18 for H0 and Om0; fixed constants for w0 and lamb",
        },
...
def main():
    ...
    print(f"FIXED_H0=Planck18.H0.value={FIXED_H0}")
    print(f"FIXED_OM0=Planck18.Om0={FIXED_OM0}")
    print(f"FIXED_W0={FIXED_W0}")
    print(f"FIXED_LAMB={FIXED_LAMB}")
    ...
    # -------------------------
    # Priors: mass-only inference.
    # Only mu_m and sigma_m are sampled. Cosmology and redshift evolution are fixed.
    # -------------------------
    priors = PriorDict()
    priors["mu_m"] = Uniform(minimum=0.8, maximum=1.8, name="mu_m", latex_label=r"$\mu_m$")
    priors["sigma_m"] = Uniform(minimum=0.005, maximum=0.5, name="sigma_m", latex_label=r"$\sigma_m$")
    priors["H0"] = DeltaFunction(peak=FIXED_H0, name="H0")
    priors["Om0"] = DeltaFunction(peak=FIXED_OM0, name="Om0")
    priors["w0"] = DeltaFunction(peak=FIXED_W0, name="w0")
    priors["lamb"] = DeltaFunction(peak=FIXED_LAMB, name="lamb")
